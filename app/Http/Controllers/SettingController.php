...
{
    // dd($request->all(), $request->file('notification_sounds'));
    $request->validate([
        'user_id' => 'required',
        'notification_sounds.*' => 'nullable|mimes:mp3,wav,ogg|max:10240',
    ]);

    // Purana record lao
    $setting = Setting::where('user_id', $request->user_id)->first();
    $savedSounds = $setting ? json_decode($setting->notification_sounds, true) : [];

    $files = [];

    // 4 slots check karo
    for ($i = 0; $i < 4; $i++) {
        if ($request->hasFile("notification_sounds.$i")) {
            $file = $request->file("notification_sounds.$i");

            $filename = 'notif_' . $request->user_id . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            $path = $file->storeAs('notification_sounds', $filename, 'public');

            $files[$i] = 'storage/' . $path;
        } else {
            // Agar nayi file nahi di gayi to purana hi use karo
            $files[$i] = $savedSounds[$i] ?? null;
        }
    }
    ...
    Setting::updateOrCreate(
        ...
        ['notification_sounds' => json_encode($files)]
    );

    return back()->with('success', 'Notification sounds updated successfully!');
}
